...
def practice_slider():
    ...
    slider = Slider(start=0, end=10, value=0, step=0.25, title='Valor actual',
                    bar_color='black')
    source = ColumnDataSource(data=dict(x=[0.0],y=[0.0],txt=['Mueva el control deslizante a 5.0'],color=['blue']))
    ...
    callback = CustomJS(args=dict(source=source,s=slider),
                        ...
                        if (s_val == 5.0) {
                            ...
                            txt[0] = 'Mueva el control deslizante a 5.0'
                            col[0] = 'blue'
                        }
                        source.change.emit();
                        """)
...
def lightcurve_slider(free_radius=True,free_impact=False,savePlot=False):
    ...
    plot2.xaxis.axis_label = "Distancia X (Radios Terrestres)"
    plot2.yaxis.axis_label = "Distancia Y (Radios Terrestres)"
    plot2.xaxis.axis_label_text_font_size = axes_font_size
    plot2.yaxis.axis_label_text_font_size = axes_font_size
    plot2.title.text = 'Vista de la Estrella'

    t_slider = Slider(start=-1.5, end=1.5, value=0, step=0.01, title='Tiempo al Centro del Tránsito (horas)',
                      bar_color='black')
    r_slider = Slider(start=0.0, end=1.5, value=r[0], step=.01, title="Radio (Radios Terrestres)",
                      bar_color='black')
    b_slider = Slider(start=0.0, end=1.1, value=0.2, step=0.01, title="Parámetro de Impacto",
                      bar_color='black')
...
def scattering_slider(savePlot=False,plots=['planet','spectrum','lightcurve']):
    ...
    plot1.title.text = 'Vista del Planeta'
    plot1.xaxis.axis_label = "Tamaño X (Radios Terrestres)"
    plot1.yaxis.axis_label = "Tamaño Y (Radios Terrestres)"
    plot1.xaxis.axis_label_text_font_size = axes_font_size
    plot1.yaxis.axis_label_text_font_size = axes_font_size
    ...
    plot2.xaxis.axis_label_text_font_size = axes_font_size
    plot2.yaxis.axis_label_text_font_size = axes_font_size
    plot2.scatter('w','rad',source=source,line_width=3,fill_color='colors',size=12)
    ...
    plot3.xaxis.axis_label = "Tiempo (horas)"
    plot3.yaxis.axis_label = "Brillo Relativo (%)"
    plot3.xaxis.axis_label_text_font_size = axes_font_size
    plot3.yaxis.axis_label_text_font_size = axes_font_size
    plot3.title.text = 'Gráfica de la Curva de luz'
    
    t_slider = Slider(start=0, end=0.3, value=0.3, step=0.01, title='Espesor Atmosférico')
...
def transmission_spec_slider(mysteryNum=1,savePlot=False):
    ...
    plot1.xaxis.axis_label = "Tiempo (horas)"
    plot1.yaxis.axis_label = "Brillo Relativo (%) - Compensación"
    plot1.xaxis.axis_label_text_font_size = axes_font_size
    plot1.yaxis.axis_label_text_font_size = axes_font_size
    ...
    slider_list = []
    for waveInd in np.arange(nWave):
        thisTitle = "Radio (Radios Terrestres) en {:.2f} micrones".format(w[waveInd])
...
convertDict = {'H2O':'Vapor de Agua','CH4':'Metano','CO2':'Dióxido de Carbono','Cloudy':'Nublado'}

def example_spectra(atmospheres=['H2O','CH4','CO2','Sin atmósfera'],savePlot=False):
    ...
    for ind, atmosphere in enumerate(atmospheres):
        ...
        plot1.xaxis.axis_label = 'Longitud de Onda (micrones)'
        plot1.yaxis.axis_label = 'Tamaño (Radios Terrestres)'
        plot1.toolbar_location = None
